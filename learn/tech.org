...
1. 安装 yum 包
   #+BEGIN_SRC sh
     ...
     # MySQL 编译包
     yum install -y openssl-devel gettext-devel bison-devel ncurses-devel wget make gcc g++
   #+END_SRC
2. gcc 安装 [[https://gcc.gnu.org/install/][GCC]]
   #+BEGIN_SRC sh
     # 下载 gcc 的依赖
     ...
     make install
   #+END_SRC
3. cmake 安装 [[https://cmake.org/install/][CMake Install]]
   #+BEGIN_SRC sh
     # 下载代码
