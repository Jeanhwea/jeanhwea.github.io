...
#+LATEX_HEADER: \usepackage[left=2.6cm, right=2.6cm, top=3.0cm, bottom=3.0cm]{geometry}
...
#+BEGIN_SRC sh
  ...
  sudo apt-get install build-essential libncurses-dev autoconf make gcc \
       ...
       libncurses5-dev libxml2-dev libgnutls28-dev libtree-sitter-dev
#+END_SRC

编译安装，emacs 29 以后才支持 tree-sitter
#+BEGIN_SRC sh
  cd emacs
  ./autogen.sh
  ./configure --prefix=/opt/emacs --with-tree-sitter
...
安装 MySQL 的 man 手册
#+BEGIN_SRC sh
  tar -xzvf refman-8.0-en.man-gpl.gz
  sudo mv refman-8.0-en.man /usr/share/man/man8/
  sudo mandb
#+END_SRC
...
1. 参考资料 [[https://dev.mysql.com/doc/refman/8.0/en/source-installation.html][源代码安装文档]] | [[https://dev.mysql.com/doc/refman/8.0/en/making-trace-files.html][TRACE 文件]] | [[https://dev.mysql.com/doc/refman/8.0/en/using-gdb-on-mysqld.html][--gdb]] | [[https://dev.mysql.com/doc/dev/mysql-server/8.0.30/PAGE_OPT_TRACE.html][The Optimizer Trace]]
   #+BEGIN_SRC sh
     # freebsd13
     pkg install gmake pkgconf ncurses
   #+END_SRC
2. 配置文件 ~/etc/my.cnf~
   #+BEGIN_SRC conf
     # -*- mode: conf; -*-
...
2. 编译源代码
   #+BEGIN_SRC sh
     # 准备代码编译目录, 输出目录为 build
     mkdir build
     cd build
     cmake -DCMAKE_INSTALL_PREFIX=/opt/mysql -DWITH_BOOST=/opt/boost/boost_1_77_0 -DWITH_DEBUG=1 -DWITH_UNIT_TESTS=0 ..
     # 编译代码
     cd build
     make -j8
     make install
...
2. 清华镜像 [[https://mirrors.tuna.tsinghua.edu.cn/help/homebrew-bottles/][tuna]]
   ...
   #+END_SRC
*** CLI
1. zoxide 工具 | [[https://github.com/ajeetdsouza/zoxide][link]]
   #+BEGIN_SRC sh
     # install
   ...
   #+END_SRC
*** LSP
Java LSP 服务 | [[https://download.eclipse.org/jdtls/milestones/1.49.0/][jdtls-download-link]]
#+BEGIN_SRC sh
  # 安装 jdtls
...
#+BEGIN_EXAMPLE
  $ ~/bin
  $ ~/bin ls
  bkv  ggg  jdt  jdtls  logisim  logisim-generic-2.7.1.jar  mount-ub22  play
  $ ~/bin ls jdt
  bin            config_linux      config_mac      config_ss_linux      config_ss_mac      config_ss_win  features
  configuration  config_linux_arm  config_mac_arm  config_ss_linux_arm  config_ss_mac_arm  config_win     plugins
  $ ~/bin cat jdtls
  #!/usr/bin/env sh
...
4. GDB 单页文档 | [[https://sourceware.org/gdb/current/onlinedocs/gdb][link]]
...
**** 修改源码路径
#+BEGIN_EXAMPLE
  set substitute-path /src/old /src/new
#+END_EXAMPLE
...
#+BEGIN_EXAMPLE
  ...
  set logging on
  # Replace "filename.txt" with the desired log file name
  set logging file filename.txt
  # This allows overwriting the log file if it already exists
  set logging overwrite on
#+END_EXAMPLE

**** 修改 args 和 env
#+BEGIN_EXAMPLE
  set args aaa bbb ccc
  show args
  set env ENV1=xxx
#+END_EXAMPLE
...
**** 反向调试
一些配置信息
1. show record full insn-number-max 查看可以记录执行状态信息的最大指令个数，默认
   是 200000
2. set record full insn-number-max limit 设置可以记录执行状态信息的最大指令个数
3. set record full insn-number-max unlimited 记录所有指令的执行状态信息
...
(gdb) rn            # reverse next
(gdb) rs            # reverse step
(gdb) rsi           # reverse step instruction
#+END_SRC
...
#+END_QUOTE
参考官网说明 [[https://sourceware.org/gdb/wiki/PermissionsDarwin][gdb PermissionsDarwin]]
...
*** conda
#+BEGIN_SRC sh
  conda create --name elm
  conda create --name elm python=3.10
  conda remove --name elm --all
  conda env create --file environment.yml
  conda env list
#+END_SRC

*** uv
操作网站参考 | [[https://www.yangyanxing.com/article/uv-proxy-setting.html][link]]
1. UV_PYTHON_INSTALL_MIRROR：用于设置使用 uv python install 命令下载安装 Python 时的镜像。
1. UV_DEFAULT_INDEX：用于设置使用 uv add 命令安装第三方包时的镜像。
1. UV_INDEX：用于设置额外的安装镜像，如公司内部的包索引地址。
...
** Rust
*** 安装配置及更新
1. 设置 rustup 国内镜像 | [[http://rsproxy.cn/#getStarted][link]]
   #+BEGIN_SRC sh
...
2. 安装 rust
   ...
   #+END_SRC
3. 配置 crates 的镜像源代码 ~/.cargo/config
   #+BEGIN_SRC conf
     # ~/.cargo/config
...
附：原始安装指令 [[https://www.rust-lang.org/tools/install][link]]
#+BEGIN_SRC sh
...
*** 使用笔记
更多参考 [[https://course.rs/basic-practice/base-features.html][course.rs]]
...
*** sshfs 挂载远端的文件系统
#+BEGIN_SRC sh
  sshfs hostname:/path/to/remote /local/folder
#+END_SRC
...
#+BEGIN_EXAMPLE
  $ cat /etc/rc.conf
  hostname="bsd23"
  ifconfig_em0="inet 192.168.0.223 netmask 255.255.255.0"
...
*** samba 网络系统挂载
临时挂载，思路是安装 cifs 工具，然后使用命令挂载
#+BEGIN_SRC sh
  yum install -y cifs-utils
  mkdir -p /mnt/data
  mount -t cifs -o username="guest",password="" //192.168.100.10/share/ /mnt/data
  # 添加一些权限
  mount -t cifs -o username="guest",password="",rw,file_mode=0777,dir_mode=0777,uid=0,gid=0,vers=2.0 //192.168.100.10/share/ /mnt/data
#+END_SRC

永久挂载，需要将配置写入 /etc/fstab
#+BEGIN_SRC conf
  //192.168.100.10/share/ /mnt/data       cifs     defaults,username="guest",password="",rw,file_mode=0777,dir_mode=0777,uid=0,gid=0,vers=2.0     0       0
#+END_SRC

** UOS
*** docker/firewalld
同网络内，容器间能 ping 但访问监听端口时，提示 'No route to host'
...
创建子网
#+BEGIN_SRC sh
    docker network create --subnet=172.77.0.0/16 hutnet
#+END_SRC

#+RESULTS:
: e28c2ce8317a3b12882cf5f1b08aa2b87ebd50305e6bacd9e0957caaa0f9aee9
...
#+RESULTS:
| NETWORK      | ID     | NAME   | DRIVER | SCOPE |
| 8549bacddc39 | bridge | bridge | local  |       |
| 8a8a365252ea | host   | host   | local  |       |
| e28c2ce8317a | hutnet | bridge | local  |       |
| cf87d5217c45 | none   | null   | local  |       |

删除子网
#+BEGIN_SRC sh
    docker network rm hutnet
#+END_SRC
...
#+BEGIN_SRC conf
  ...
  location /gitlab/ {
      ...
      proxy_pass   http://172.77.0.2:80/gitlab/;
  }
#+END_SRC

*** gitlab-ce 14 初始密码无法登录的问题
gitlab-ce 安装好后，root 用户的初始密码放在
/etc/gitlab/initial_root_password 文件中，但这个密码仍然登录失败。
...
通过 CMAKE_EXPORT_COMPILE_COMMANDS 来导出编译命令 [[https://clangd.llvm.org/installation][clangd]] | [[https://cmake.org/cmake/help/latest/variable/CMAKE_EXPORT_COMPILE_COMMANDS.html][doc]]
#+BEGIN_SRC sh
  # output 是编译路径
...
*** Ubuntu 22.04 添加开机启动脚本
1. 在目录/etc/init.d/目录新建脚本
   #+BEGIN_SRC sh
     sudo vim /etc/init.d/init.sh
   #+END_SRC
2. 脚本内容
   #+BEGIN_SRC sh
     #!/bin/bash
...
4. 添加启动脚本
   #+BEGIN_SRC sh
     sudo update-rc.d init.sh defaults 90
   #+END_SRC
5. 查看服务列表
   #+BEGIN_SRC sh
...
6. 测试是否生效#
   #+BEGIN_SRC sh
     sudo service init.sh start
     sudo service init.sh stop
   #+END_SRC
7. 删除任务
   #+BEGIN_SRC sh
     sudo update-rc.d -f init.sh remove
   #+END_SRC
...
*** 挂载共享服务器
#+BEGIN_SRC sh
  echo 'pass' | sudo -S mount -t drvfs '\\hostname\共享盘' /local/folder
#+END_SRC
...
** freebsd
*** 参考链接
1. HandBook [[https://docs.freebsd.org/en/books/handbook][en]] | [[https://docs.freebsd.org/zh-cn/books/handbook][zh]]
2. FreeBSD 从入门到跑路 | [[https://book.bsdcn.org/][link]]
...
1. 磁盘分区格式化
   #+BEGIN_EXAMPLE
     ...
     =>       40  419430320  da1  GPT  (200G)
              40       2008       - free -  (1.0M)
            2048  419426304    1  freebsd-ufs  (200G)
...
2. 创建文件系统
   #+BEGIN_EXAMPLE
     # newfs -U /dev/da1p1
     /dev/da1p1: 204798.0MB (419426304 sectors) block size 32768, fragment size 4096
     ...
     super-block backups (for fsck_ffs -b #) at:
      ...
      ...
      416145792, 417426240, 418706688
     # mkdir /data
     # mount /dev/da1p1 /data
     # df
      Filesystem         1K-blocks    Used     Avail Capacity  Mounted on
     zroot/ROOT/default  60852520 2060144  58792376     3%    /
     devfs                      1       0         1     0%    /dev
     ...
     /dev/da1p1         203121560       8 186871828     0%    /data
     #
   #+END_EXAMPLE
3. 配置开机挂载 /etc/fstab
   - nosuid 禁止设置 uid
   #+BEGIN_EXAMPLE
     /dev/da1p1	/data		ufs	rw,nosuid	2	2
   #+END_EXAMPLE

*** 用户的一些配置
1. 将 admin 添加到 wheel 组, /etc/group
   - freebsd 的 root 默认不可 ssh 远程，在机器上直接操作
2. 编辑 sudoers 文件，添加 admin 到 sudoer
   #+BEGIN_SRC sh
     # 安装 sudo 命令
     pkg install sudo
...
#+BEGIN_SRC sh
  # 配置调试环境
  ./configure --prefix=/opt/pgsql --enable-cassert --enable-debug CFLAGS="-ggdb -O0 -fno-omit-frame-pointer"
  # 编译, 安装
  make -j8
  make install
#+END_SRC
...
#+BEGIN_SRC sh :results output :exports both
  ...
  jinghuihu        80299   0.0  0.1 34309108  14920   ??  Ss   10:41AM   0:00.13 /opt/pgsql/bin/postgres -D /opt/pgsql/data
#+END_SRC
...
#+BEGIN_SRC text
  ...
  Executable module set to "/opt/pgsql/bin/postgres".
...
*** Host Mapping Reverse Proxy Example
1. token from https://help.sonatype.com/en/docker-repository-reverse-proxy-strategies.html
#+BEGIN_SRC conf
  http {
...
** Tex
*** texlive 2023 安装
参考一个简短的说明文档 [[https://mirrors.tuna.tsinghua.edu.cn/CTAN/info/install-latex-guide-zh-cn/install-latex-guide-zh-cn.pdf][link]]
...
#+BEGIN_SRC sh
  mkdir /mnt/texlive
  mount -o loop texlive2023-20230313.iso /mnt/texlive
#+END_SRC
...
1. clang 是 llvm 的 c/c++ 编译前端
  ...
  #+END_SRC
2. lli 会解释或以 JIT 的形式运行 .bc 和 .ll 程序。
   - .bc 是 LLVM IR 的二进制格式 (Bit Code)
   - .ll 是 LLVM IR 的可读代码格式 (Low Level)
   #+BEGIN_SRC sh
     ...
     lli main.ll
   #+END_SRC
3. llvm-link 用于链接其它库的 .ll 和 .bc 文件，最终输出单个可执行的 main.ll
   #+BEGIN_SRC sh
     # 1. 分别导出 mylib.c 和 main.c 对应的的 .ll 文件
     clang -emit-llvm -S mylib.c -o mylib.ll
     clang -emit-llvm -S main.c -o main.ll
     # 2. 使用 llvm-link 将两个文件链接，生成新的 IR 文件
     llvm-link main.ll mylib.ll -S -o out.ll
     # 3. 用 lli 解释运行
     lli out.ll
   #+END_SRC
4. llc 可以将 .ll 形式的 IR 编译成指定体系结构的汇编
   #+BEGIN_SRC sh
     llc main.ll -o main.s
     llc -O3 main.ll -o main.s
   #+END_SRC
5. opt 是 LLVM 模块化的优化器和分析器
...
** Android
*** AOSP 环境搭建
参考清华大学镜像  [[https://mirrors.tuna.tsinghua.edu.cn/help/git-repo/][git-repo]] | [[https://mirrors.tuna.tsinghua.edu.cn/help/AOSP/][AOSP]]
...
* 机器学习
** NVIDIA
1. [[https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html][nvidia-container-toolkit]]
2. 启动 pytorch 运行环境, 参考 [[https://catalog.ngc.nvidia.com/orgs/nvidia/containers/pytorch][nvcr]] 配置环境
#+BEGIN_SRC sh
