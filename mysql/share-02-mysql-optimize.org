...
*** 逻辑变换
1. 否定消除：对于多个表达式的和取或析取范式前面有否定的情况，应将关系条件分解成
   一个一个的，将外面的 NOT 消除；
2. 等值常量传递：利用了等值关系的传递特性，为了能够尽早执行下推运算
3. 常量表达式计算：对于能够立刻计算出结果的表达式，直接计算结果，并将结果与其他
   条件尽量提前化简

[[file:../static/image/2023/0302/113149.png]]
...
*** cost 常量
MySQL 8.0 的 cost 配置常量如下所示, 详见 [[https://dev.mysql.com/doc/refman/8.0/en/cost-model.html][Cost Model]]
服务端的 cost
...
** Misc
*** Invisible Indexes
[[https://dev.mysql.com/doc/refman/8.0/en/invisible-indexes.html][Invisible Indexes]] 是一种不被优化器使用的索引
#+BEGIN_SRC sql :results output :exports both
