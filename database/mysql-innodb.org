...
*** 隔离级别
    ...
    3. READ COMMITTED (RC) 读已提交
       - [[https://dev.mysql.com/doc/refman/5.7/en/glossary.html#glos_consistent_read][一致读]] (Consistent Read) 读的是所有事务的最新的快照, 这种读法也称当前读
         (Current Read)
       ...
       - RC 可能产生系列影响
         ...
         + 对于 ~UPDATE~ 语句, 如果匹配行被锁住, InnoDB 会执行 **半一致读**
           (Semi-Consistent Read), 即返回最近一条匹配行, **只有当匹配行必须更新
           时**, MySQL 会再次读取它, 这时 InnoDB 才会锁定它, 或者阻塞等待
