...
*** 索引类型
    ...
    3. Function-based Indexes 经常对某个字段做查询的时候是带函数操作的
       #+BEGIN_SRC sql
         -- 函数索引
...
*** 外键约束
    ...
    | ~update no action~ | 如果从表中没有引用主表的行 | 如果更新后新的外键仍然引用旧的外键                |
...
*** 块 Block
    ...
    #+END_SRC
    注意：数据库块大小一旦启动后不要轻易修改，11g 中默认的块大小为 8K。在创建新
    的表空间时可以通过参数指定表空间的块大小，一旦表空间创建，块大小不要修改
    ...
    Oracle 允许手工管理表分配块的大小，一旦指定 ~pctfree~ 参数，Oracle 在创建块
    时要使得分配的块不低于 ~pctfree~ 的值
    #+BEGIN_SRC sql
...
*** 区 Extent
    ...
    区的管理可以是手动的，也可以是自动的。如果需要统一管理在创建表空间时指定扩展
    区大小为 1M
*** 段 Segment
    Oracle 的段一般对应与一张表
...
** 日期格式
   ...
   方案三：在选取时间日期字段时显式使用 ~to_char~ 函数来格式化时间
   #+BEGIN_SRC sql
     select
...
*** 创建表空间
    Oracle 在创建用户前最好为当前用户显式创建表空间，否则创建的新用户会使用默认
...
** 将服务器字符集从 UTF8 强转成 GBK
   修改数据库字符集最好放在安装数据库的最初阶段，如果修改一个已经运行的数据库
   可能会发生一些不可预料的错误
   #+BEGIN_SRC sql
     -- 本操作必须使用 DBA 权限
...
** 启动 sqlplus
   ...
   当命令中的密码字段中含有 @ 字段时需要将 password 字段用双引号包起来，如下：
   #+BEGIN_SRC shell
...
** 确定高速缓冲区命中率
   ...
   #+BEGIN_SRC sql
     SELECT
       100 * (1 - ((PHYSICAL.VALUE - DIRECT.VALUE - LOBS.VALUE) / LOGICAL.VALUE)) AS "Buffer Cache Hit Ratio (%)"
     FROM
       V$SYSSTAT PHYSICAL,
       V$SYSSTAT DIRECT,
       V$SYSSTAT LOBS,
