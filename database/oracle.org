...
*** 数值函数
    ...
    - ~WIDTH_BUCKET(n, min_value, max_value, num_buckets)~ : 使用 ~WIDTH_BUCKET~
      可以根据输入参数创建等长的段。范围 ~min_value~ 到 ~max_value~ 被分
      为~num_buckets~ 节，每节有相同的大小。返回 n 所在的那一节。如果 n 小
      于~min_value~，将返回 0，如果 n 大于或等于 ~max_value~，将返回~num_buckets~
      + 1. ~min_value~ 和 ~max_value~ 都不能为 NULL，~num_buckets~ 必须是一个正
      整数。如果 n 是 NULL，则返回 NULL。
