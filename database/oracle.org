...
** 字符串函数
   - ~chr(n)~ : 数字转字符
   - ~concat(str1, str2)~ : 字符串拼接，也可以使用 =||=
   - ~lower(str)~
   - ~upper(str)~
   - ~replace(str, search_str, repl_str)~ : 在字符串 =str= 中查找 =search_str=
     并替换成 =repl_str=
   - ~substr(str, m, n)~ : 子串
   - ~length(str)~ : 字符串长度
   - ~to_date(str)~ : 转成数据库中 date 类型

** 日期函数
   - ~add_month~
   - ~month_between~
   - ~next_day~
   - ~to_char~ : 格式化日期。 ~to_char(sysdate, 'YYYY-MM-DD HH:MM:SS')~
