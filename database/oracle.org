...
**** 逻辑存储结构
     ...
     - *区* (Extents) 由定长的逻辑连续 Data blocks 构成。Extends 一次性分配，用来存储特
       定类型的信息
     - *段* (Segments)  由一系列的 Extents 构成，可以对应一些用户的对象，例如：表、索引、
       undo data 和 temporary data
     - *表空间* (Tablespace) 一个 Database 被分成多个逻辑的 Tablespace 单元。Tablespace 是
