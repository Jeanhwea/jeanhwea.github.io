...
** 创建表空间
   创建数据表空间
   #+BEGIN_SRC sql
     CREATE TABLESPACE AVIC_P01 DATAFILE
       '/u01/app/oracle/oradata/ora11g/AVIC_P01.ORA'
   ...
   创建临时表空间
   #+BEGIN_SRC sql
     CREATE TEMPORARY TABLESPACE AVIC_T01 TEMPFILE
       '/u01/app/oracle/oradata/ora11g/AVIC_T01.ORA'
...
** 创建用户
   #+BEGIN_SRC sql
     ...
     CREATE USER AVICTOR
       IDENTIFIED BY VALUES 'Pa$$word'
       DEFAULT TABLESPACE AVIC_P01
       TEMPORARY TABLESPACE AVIC_T01
       PROFILE DEFAULT
       ACCOUNT UNLOCK;
     ...
     ALTER USER AVICTOR IDENTIFIED BY 'Pa$$word';
   #+END_SRC
