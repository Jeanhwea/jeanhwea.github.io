...
** 创建表
   ...
   - 存储引擎: ~ENGINE=engine_name~ 表在管理数据时采用的不同的数据结构，结构不同
     会导致处理方式、提供的特性操作等不同。常见的引擎：InnoDB MyISAM Memory/Heap
...
**** 整型
     - ~int(M)~ : M 表示总位数
     - 默认存在符号位, 使用 ~unsigned~ 来显式指定成无符号存储
...
**** 字符串
     ...
     - varchar 变长字符串，速度慢，但节省空间 M 表示能存储的最大长度，此长度是字
       ...
       然后还需两个字节来存 放字符串的长度，所以有效长度是 65535-1-2=65532 字节。
       例：若一个表定义为
     ...
     - text 非二进制字符串（字符串）tinytext, text, mediumtext, longtext
       text 在定义时，不需要定义长度，也不会计算总长度。text 类型在定义时，不可
       给 default 值
     - binary, varbinary 类似于 char 和 varchar，用于保存二进制字符串，也就是保
       存字节字符串而非字符串。char, varchar, text 对应 binary, varbinary,
       blob.
...
** 查询数据
   ...
   - select_expr:
     ...
     + 为了简化列或避免多个列标识符重名，可以对 =select_expr= 的列起别名。 使用
       ~AS~ 关键字，也可以省略 ~AS~
   - FROM 子句
...
* 数据优化：索引
** 索引简述
   1. MySQL 的 InnoDB 支持 B+树索引和哈希索引
   2. 哈希索引存在缺点是不支持范围查找
   3. 走索引最简单的原则是最大前缀
      - 例如：如果建了索引 ~a, b, c~,
        #+BEGIN_SRC sql
          -- 走索引
          select * from tab01 where a = 1;
          select * from tab01 where a > 1;
          select * from tab01 where a = 1 and b = 5;
          -- 不走索引
          select * from tab01 where b = 1;
        #+END_SRC
      - 对字段隐式类型转换和计算也不走索引
        #+BEGIN_SRC sql
          -- 不走索引
          select * from tab01 where a + 4 > 1;
        #+END_SRC
   4. 使用索引时需要权衡排序查找和回表的两者的效率
      - 例如：如果建了索引 ~a, b, c~,
        #+BEGIN_SRC sql
          -- 走索引
          select a from tab01 where a > 1;
          -- 不走索引, e, f的回表代价较大
          select a,e,f from tab01 where a > 1;
        #+END_SRC
   5. 联合索引中，叶子节点只记录联合的字段和主键，对于 ~select *~ 可能需要回表搜
      索
...
** 合并多个查询结果 ~UNION~
    ...
    #+END_SRC
    - 默认 DISTINCT 方式，即所有返回的行都是唯一的。建议对每个 SELECT 查询加上小
      括号包裹。
...
**** 创建存储函数
     ...
     - 一个函数应该属于某个数据库，可以使用 db_name.function_name 的形式执行当前
       函数所属数据库，否则为当前数据库
...
*** 存储过程
    存储过程是一段代码（过程），存储在数据库中的 SQL 组成。一个存储过程通常
    用于完成一段业务逻辑，例如报名，交班费，订单入库等。而一个函数通常专注于某个
    功能，是为其他程序服务的，需要在其他语句中调用函数才可以，而存储过程不能被其
    他调用，是自己执行通过 ~CALL~ 执行。
...
** 权限层级
   要使用 GRANT 或 REVOKE，您必须拥有 GRANT OPTION 权限，并且您必须拥有您正在授
   予或撤销的权限。
