...
*** 绑定事件
    ...
    3. ~addEventListener/removeEventListener~ 添加和移除事件
       ...
       - options 可选项，
         ...
         - phase 布尔值。默认为假，表示 bubbling 阶段的事件；真表示 capturing 阶
           段的事件
...
** Promise
   ...
   Promise 对象有 =state= 和 =result= 这两个属性，满足如下状态图
   [[file:../static/image/2019/08/promise-states.png]]

   Promise 有 =then= 和 =catch= 两个成员函数
   - =then= 方法可以用来实现链式调用
     + ~then~ 返回普通值时，会立马返回结果到下一个 handler
     + 当 ~then~ 方法返回一个新的 Promise 对象时，直到调用完毕后才返回结果
   - =catch= 方法用来错误处理
