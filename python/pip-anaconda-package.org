...
** 依赖包管理
   #+BEGIN_SRC sh
     ...
     # 安装环境依赖到环境中
     conda install --yes tensorflow==1.13.1
     conda install --yes --file requirements.txt

     # 下载安装包, 缓存在 ~/.conda/pkgs/ 文件夹中
     conda install --download-only tensorflow==1.13.1
   #+END_SRC
...
** 下线 Anaconda 安装包
   直接去清华镜像源中下载 [[https://mirrors.tuna.tsinghua.edu.cn/anaconda/archive/][Anaconda]]
