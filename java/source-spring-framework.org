...
** 单例 Bean 工厂实现: ~DefaultSingletonBeanRegistry~ 类
   ...
   2. earlySingletonObjects: 二级缓存早期创建的单例 Bean 对象
      ...
      - 这样做，即使单例 Bean 在创建过程中，调用 ~getBean(...)~ 方法依然可以获取
        到所依赖的 Bean 对象
      - 另外该方案用来循环检测，避免循环依赖
...
*** 获取单例 Bean : ~getSingleton(...)~ 方法
    ...
    获取单例 Bean 的总逻辑需要结合 ~doGetBean(...)~ 方法看，它的逻辑如下
    1. 首先尝试从 singletonObjects 哈希表中获取，获取成功直接返回 Bean 对象
    2. 如果 singletonObjects 获取失败，尝试从 earlySingletonObjects 哈希表中获取，
       如果成功，返回 Bean 对象
    3. 如果 earlySingletonObjects 获取失败，则需要创建单例 Bean 对象
       - 调用 ~ObjectFactory~ 的 ~getObject(...)~ 方法来创建 Bean 对象
       - 创建成功的，调用 ~addSingleton(...)~ 来处理添加新创建 Bean 对象逻辑
         #+BEGIN_SRC java
         ...
         #+END_SRC
       - ~addSingleton(...)~ 将新创建的 Bean 对象放入 ~singletonObjects~, 并从
         ~singletonFactories~ 中移除 ~ObjectFactory~ 对象

*** 从 Bean 实例获取 Bean 对象: ~getObjectForBeanInstance(...)~ 方法
