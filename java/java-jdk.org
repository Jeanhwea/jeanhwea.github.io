...
** 对象初始化和销毁
   ...
   2. 一般 ~finalize()~ 方法只会在对象内存回收前被调用一次，我们不需要显示让对象
      销毁, 有时候会显示将将引用置空来帮助 GC
   3. 而是让 GC 来自动处理对象的析构，并且当需要阻止对象消亡时，就需要显示将保护
      的对象通过 ~finalize()~ 挂载到 GCRoot 上
...
** String 的不可变性
   ...
   2. 为什么要把 String 类设计成不可变类，是它的用途决定的
   ...
   3. ~javac~ 编译可以对字符串常量直接相加的表达式进行优化
