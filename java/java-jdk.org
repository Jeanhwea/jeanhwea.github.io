...
** 错误和异常
   1. 错误 ~java.lang.Error~ 是 Throwable 的子类
      ...
      - 是在恢复不是不可能但很困难的情况下的一种严重问题；比如内存溢出，不可能指望
        程序能处理这样的情况
...
** 对象初始化和销毁
   ...
   2. 对象初始化块每次对象实例化都调用一次，并且是在构造器调用前调用
   #+BEGIN_SRC java
   ...
   3. 我们不需要显式让对象销毁, 而是让 GC 来自动处理对象的析构，并且当需要阻止对
      象消亡时，就需要显式将保护的对象通过 ~finalize()~ 挂载到 GCRoot 上
   4. 有时候会显式将引用置空来帮助 GC
...
*** 封装
    ...
    2. 封装的好处如下
       ...
       - 允许设计者可以改变类内部的工作方式而不用担心会影响到客户端程序员
...
*** 泛型的注意点
    1. Java 中泛型必须是对象，像 ~int~, ~long~, ~double~ 等是不可以定义到泛型中，
       需要使用对应的包装类
    2. 泛型只是在编译时才存在，编译完成后就没有泛型留在字节码中。所以对应使用
       泛型作为传参的其实是一个方法，例如
       #+BEGIN_SRC java
...
** 反汇编器 javap
   javap 是 JDK 自带的反汇编器，可以反汇编 java 的类
