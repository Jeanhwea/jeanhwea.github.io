...
** 修改用户信息
   #+BEGIN_SRC shell
     ...
     adduser user
     # 删除用户：1. 调用 userdel 删除用户；2. 删除用户的家目录
     deluser user
...
*** 查看文件信息
    #+BEGIN_SRC shell
      ...
      less -n+120 /path/to/file
      less +120 /path/to/file
    #+END_SRC
...
*** 文件复制、移动和删除
    #+BEGIN_SRC shell
      ...
      # 创建软链接，dest 是所指向的文件，link 是软链接
      # 例如：ln -s /tmp ~/Public/tmp 相当于在 ~/Public 中建了一个 tmp 软链接，指向 /tmp
      ln -s dest link
    #+END_SRC
...
*** 重定向
    #+BEGIN_SRC shell
      ...
      cmd 2> file
      # 将 cmd 的 stdout 和 stderr 重定向到 file 中，2>&1 必须放在 > file 后面
      cmd > file 2>&1

      # 将 cmd 的 stdout 和 stderr 重定向到 /dev/null 中，并作为后台任务运行
      cmd >/dev/null 2>&1 &
    #+END_SRC
...
** 监控进程
   #+BEGIN_SRC shell
     ...
     # htop 和 top 类似，但是比 top 界面更友好，需要安装
     htop
...
** DPKG & APT
   #+BEGIN_SRC shell
     # 安装软件包
     dpkg -i package.bla.deb
     # 从软件包里面读取软件的信息
     dpkg -A package.bla.deb
     # 安装一个目录下面所有的软件包
...
** jq
   ...
   去除字符串的双引号，jq 在输出 JSON 时遇到字符串会添加上双引号，可以使用 -r 选项来去除双引号
   #+BEGIN_SRC text
     ~ $ echo '["a", "b", "c"]' | jq '.[]'
     "a"
     "b"
...
* 杂项
  #+BEGIN_SRC shell
    ...
    date -d @1564848200 +'%Y-%m-%d %H:%M:%S'

    # 设置命令超时，超时的时长为 10 秒，如果发送命令后 5 秒仍然运行则直接 kill
