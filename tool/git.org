...
*** 重写历史
    - ~git commit --amend~ : 将 Index 中的修改和最近一次的提交合并。并且修改提交
      信息
    - ~git rebase <base>~ : 基于 <base> 重建当前的分支
...
*** 分支操作
    ...
    - ~git checkout -b <branch>~ : 新建 <branch> 分支，并且切换到新建的分支上
...
*** 重建基
    ...
    - ~git rebase -i <base>~ : 交互式基于 <base> 重建当前分支，中间会启动编
      辑器做相应的修改
...
*** 推送远端
    ...
    - ~git push <remote> --force~ : 强制推送到远端仓库
    - ~git push <remote> --all~ : 将本地的全部分支推送到远端仓库
    - ~git push <remote> --tags~ : 将本地的全部标签推送到远端仓库， *--all* 选项
