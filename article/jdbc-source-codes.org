...
** JDBC 连接数据库流程
   ...
   3. 注意 ~Class.forName(...)~ 是兼容性的写法
      - 低版本的 JDK 中没有加载数据库驱动的代码，需要显式加载驱动
...
** SPI 连接数据库流程
   ...
   4. 使用 ~DriverManager~ 中定义了 ~registeredDrivers~ 来存储项目中引用的确定类
      ...
      #+END_SRC
   5. 调用 ~getConnection~ 方法时通过遍历所有注册的驱动来获取连接信息
      #+BEGIN_SRC java
...
** 执行 SQL: ~execute(...)~ 方法
   ...
   #+END_SRC
   1. ~applyStatementSettings(...)~ 用于设置输入参数
      - fectchSize 减少网络交互次数
      - maxRows 获取结果集的最大行数
...
** 数据库更新: ~update(...)~ 方法
   数据库更新和执行 SQL 类似，这里需要熟悉 ~PreparedStatement~ 接口，它是一个预编
   译的 SQL 语句
   #+BEGIN_SRC java
