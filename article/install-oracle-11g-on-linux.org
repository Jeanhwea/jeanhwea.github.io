...
** 数据库安装文件
   Oracle 数据库选择 11g R2 下面两个是数据库的安装文件
   - =p13390677_112040_Linux-x86-64_1of7.zip=
   - =p13390677_112040_Linux-x86-64_2of7.zip=

   将两个文件解压到同一个文件夹中，得到一个 =database= 的文件夹
   #+BEGIN_SRC shell
...
** 检验操作系统的内存
   ...
   的大小差不多，也就是 swap 也修改成 32G，否则后面装 Oracle 数据库时分配数据库
   内存是不允许超过 swap 的大小
...
** 配置 swap 分区大小
   为了操作系统能探测到 swap 的大小，显式指定 size 值，编辑 =/etc/fstab= 文件
...
** 配置内核参数
   ...
   下要求的最小值，对于实际的系统可能需要参考系统默认参数，并计算调优一些参数，
   修改默认设置
