...
* 并发概念
** 并发 vs. 并行
并发 (concurrent) 和并行 (parallel) 是两个不同的概念，并发强调是“发”（出发），并
...
2. Actor 必须只能通过消息传递进行通信。
3. 一个 Actor 可以响应消息:推出新 Actor,改变其内部状态,或将消息发送到一个或多个其他参与者。
4. Actor 可能会阻塞自己,但 Actor 不应该阻塞它运行的线程。
...
通过共享内存来通信” 理念的最有力的工具。Channel 大家都比较熟悉，就不过多赘述了。
