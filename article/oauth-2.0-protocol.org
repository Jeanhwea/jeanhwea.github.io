...
* OAuth 2.0 协议
  ...
  但不向前兼容 OAuth 1.0(即完全废止了 OAuth1.0)。 OAuth 2.0 关注客户端开发者的简
...
** 角色
   ...
   - Resource Server: 资源服务器，托管受保护资源的服务器，能够接收和响应使用访问
     令牌对受保护资源的请求
   - Client: 客户端，通常是第三方应用。使用资源所有者的授权代表资源所有者发起对受
     保护资源的请求的应用程序。术语 "客户端" 并非特指任何特定的实现特点（例如：
...
** Access Token
   Access Token 即访问令牌，Access Token 是用来访问资源的，客户端在访问资源时必
...
** Refresh Token 及刷新过程
   ...
   则需要使用 Refresh Token 申请一个新的 Access Token 。刷新 Access Token 的流程
   如下图
   ...
   5. =步骤 3= 和 =步骤 4= 重复进行，直到 Access Token 到期。如果客户端知道
      Access Token 已过期，跳到 =步骤 7= ，否则它将继续发起另一个对受保护资源的
      请求
...
*** 授权码许可请求
    ...
    - =redirect_uri= 可选的
    - =scope= 可选的。访问请求的范围
    - =state= 推荐的。客户端用于维护请求和回调之间的状态的不透明的值。当重定向用
...
** 简化许可
*** 隐式许可流程
    隐式授权类型被用于获取 Access Token（它不支持发行 Refresh Token），并对操作具体重定
...
*** Access Token 响应
    如果访问令牌请求是有效的且被授权，授权服务器颁发访问令牌以及可选的刷新令牌。如
...
*** Access Token 响应
    如果访问令牌请求是有效的且被授权，授权服务器颁发访问令牌以及可选的刷新令牌。刷新
