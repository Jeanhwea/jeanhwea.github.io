...
* Redis 实现扣减库存的途径
  因为 Redis 的并发量比 MySQL 要高，可以引入 Redis 来优化库存扣减的逻辑，从而提
  高系统的吞吐量
...
* 数据一致性的讨论
  ...
  1. 在分布式系统中要保证强一致性是不可能的
  2. 通过 BASE 理论，在互联网业务中只是要求最终一致性即可

  在保证数据最终一致的方式上，这里引入了消息队列的机制
  1. 消息队列的好处是可以慢慢处理请求，不必同步处理响应结果
  ...
  3. 目前主流的消息队列包括 RocketMQ, Kafka
     ...
     #+END_SRC
     - 如果 Broker 可能会重复发送, 消费者在实现 API 时需要保证接口的幂等性
