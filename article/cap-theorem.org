...
* CAP 定理
  ...
  2. A (Availability) 可用性, 指集群中没有宕机的节点都能正常服务
  3. P (Partition Tolerance) 分区容错性, 指集群因网络故障或节点故障问题，被切割
...
** ZK 的 Quorum
   ...
   2. 如果发生 Partition, ZK 仍然可以维持服务，因为 ZK 采用了 Quorum 机制，所以
      ZK 是偏向于 CP 的系统
   3. Quorum 是成员数达到最低投票门槛的成员集合，Quorum 具有如下属性
      - 成员只能是 Leader 或 Follower
      - 一个 Quorum 中只有一个 Leader, 其它都是支持该 Leader 的 Follower
...
** Kafka 的 Replication 机制
   ...
   3. 每个 Replica Set 会维持一个 ISR (In-Sync Replica) 名单
      ...
      - 如果 Master 挂掉，会从 ISR 中挑选出新的 Master
